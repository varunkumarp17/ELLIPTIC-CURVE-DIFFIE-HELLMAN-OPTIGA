...
int main(void)
{
	...
	mbedtls_ecdh_context ecdhalice;
	mbedtls_ecdh_init(&ecdhalice);

	 mbedtls_ecdh_context ecdhbob;
	 mbedtls_ecdh_init(&ecdhbob);

	 mbedtls_ecdh_setup(&ecdhalice, MBEDTLS_ECP_DP_SECP256R1);
	 mbedtls_ecdh_setup(&ecdhbob, MBEDTLS_ECP_DP_SECP256R1);

	 mbedtls_ecdh_gen_public(&ecdhalice.grp,&ecdhalice.d,&ecdhalice.Q, mbedtls_ctr_drbg_random, &ctr_drbg);

	 mbedtls_ecdh_gen_public(&ecdhbob.grp,&ecdhbob.d,&ecdhbob.Q, mbedtls_ctr_drbg_random, &ctr_drbg);



	 printf("\r\nAlice Private key: \r\n");
	 print_mpi_data(&ecdhalice.d);
	 printf("\r\nAlice Public Key: \r\n");
	 print_ecp_point_data(&ecdhbob.Q,&ecdhbob.grp);


	 printf("\r\nBob Private key: \r\n");
	 print_mpi_data(&ecdhbob.d);
	 printf("\r\nBob Public Key: \r\n");
	 print_ecp_point_data(&ecdhalice.Q, &ecdhalice.grp);

	mbedtls_ecdh_compute_shared(&ecdhalice.grp,&ecdhalice.z,&ecdhbob.Q,&ecdhalice.d, mbedtls_ctr_drbg_random, &ctr_drbg);
	mbedtls_ecdh_compute_shared(&ecdhbob.grp,&ecdhbob.z,&ecdhalice.Q,&ecdhbob.d, mbedtls_ctr_drbg_random, &ctr_drbg);


	if(mbedtls_mpi_cmp_mpi(&ecdhalice.z,&ecdhbob.z)==0)
	{
	printf("Alice Shared Secret");
	print_mpi_data(&ecdhalice.z);
	printf("Bob Shared Secret");
	print_mpi_data(&ecdhbob.z);

	}
	else
	{
			printf("Shared key compute failed");
	}
}
